...
impl sys::SocketAddrV4 for SocketAddrV4 {
    ...
    fn addr(&self) -> &Ipv4Addr {
        unsafe {
            &*(&self.0.sin_addr as *const libc::in_addr as *const Ipv4Addr)
        }
    }

    fn port(&self) -> u16 { ntoh(self.0.sin_port) }
}
...
impl sys::SocketAddrV6 for SocketAddrV6 {
    ...
    fn addr(&self) -> &Ipv6Addr {
        unsafe {
            &*(&self.0.sin6_addr as *const libc::in6_addr as *const Ipv6Addr)
        }
    }

    fn port(&self) -> u16 { ntoh(self.0.sin6_port) }

    fn flowinfo(&self) -> u32 { ntoh(self.0.sin6_flowinfo) }

    fn scope_id(&self) -> u32 { ntoh(self.0.sin6_scope_id) }
}
